...
class HrEmployee(models.Model):
    ...
    def _cron_update_employee_service(self):
        _logger.warning("run _cron_update_employee_service")
        employees = self.env['hr.employee'].search([
            ('active', '=', True)
        ])
        for employee in employees:
            employee._compute_service_duration()
            employee._compute_service_duration_display()
        _logger.warning("done _cron_update_employee_service")
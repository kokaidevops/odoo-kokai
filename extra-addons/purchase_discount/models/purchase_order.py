...
class PurchaseOrder(models.Model):
    ...
    @api.depends('has_discount_global', 'discount_type', 'discount', 'discount_fixed')
    def _compute_discount_amount(self):
        for record in self:
            record.discount_amount = (record.discount_fixed if record.discount_type == 'fixed' else record.amount_total*record.discount/100) if record.has_discount_global else 0
    ...
    @api.depends('has_discount_global', 'discount_type', 'discount', 'discount_fixed', 'discount_amount')
    def _amount_all(self):
        res = super(PurchaseOrder, self)._amount_all()
        for order in self:
            discount_amount = (order.discount_fixed if order.discount_type == 'fixed' else order.amount_total*order.discount/100) if order.has_discount_global else 0
